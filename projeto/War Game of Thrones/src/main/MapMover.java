...
public class MapMover extends Entity {
    ...
    private Vector2f lastActivation = null;

    private enum ScrollState {

        WAITING, MOVING, COMPLETE_DELAY
    }
...
//            }
    ...
    private boolean reachedDestiny() {
        ...
        if (lastActivation != null)
            reached = originalSignX != currSignX || originalSignY != currSignY || (lastActivation.x == end.x && lastActivation.y == end.y);
        else
            reached = originalSignX != currSignX || originalSignY != currSignY;
        if (reached)
            lastActivation = end;
